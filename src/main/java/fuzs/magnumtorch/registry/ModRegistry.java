...
public class ModRegistry {
    ...
    // PointOfInterestHelper::register method does some additional setup (which is not done on Forge for some reason?), so skip using our own registry
    public static final PoiType DIAMOND_MAGNUM_TORCH_POI_TYPE = PointOfInterestHelper.register(new ResourceLocation(MagnumTorch.MOD_ID, "diamond_magnum_torch"), 0, 1, ImmutableSet.copyOf(DIAMOND_MAGNUM_TORCH_BLOCK.getStateDefinition().getPossibleStates()));
    public static final PoiType EMERALD_MAGNUM_TORCH_POI_TYPE = PointOfInterestHelper.register(new ResourceLocation(MagnumTorch.MOD_ID, "emerald_magnum_torch"), 0, 1, ImmutableSet.copyOf(EMERALD_MAGNUM_TORCH_BLOCK.getStateDefinition().getPossibleStates()));
    public static final PoiType AMETHYST_MAGNUM_TORCH_POI_TYPE = PointOfInterestHelper.register(new ResourceLocation(MagnumTorch.MOD_ID, "amethyst_magnum_torch"), 0, 1, ImmutableSet.copyOf(AMETHYST_MAGNUM_TORCH_BLOCK.getStateDefinition().getPossibleStates()));
